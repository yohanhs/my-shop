...
export type UsuarioListPagedOpts = {
  page?: number;
  pageSize?: number;
  nombre?: string;
  username?: string;
  status?: string;
  /** id de rol como string, vacío = sin filtrar */
  rolId?: string;
  createdFrom?: string;
  createdTo?: string;
  updatedFrom?: string;
...
export function buildUsuarioWhere(opts: UsuarioListPagedOpts): Prisma.UsuarioWhereInput {
  ...
  const rid = typeof opts.rolId === 'string' ? opts.rolId.trim() : '';
  if (rid && /^\d+$/.test(rid)) {
    and.push({ rolId: Number(rid) });
  }
